...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	protected EntityResolver getEntityResolver() {
		/*
		   对于解析一个XML，SAX首先读取该XML文档上的声明，根据声明去寻找相应的DTD定义，以便对文档进行一个验证。默认的
		   寻找规则是通过网络(实现上就是声明的DTD的URI地址)来下载相应的DTD声明，并进行认证。过程漫长，网络中断时会报错
		   EntityResolver的作用就是项目本身就可以提供一个如何寻找DTD声明的方法，即由程序来实现寻找DTD声明的过程，可以将DTD文件
		   放到项目的某处，直接将此文档读取并返回给SAX，避免了网络来寻找相应的声明
		 */
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		...
		*/
		...
		// createReaderContext:生成xmlreader的上下文context，其中有配置好的命名空间
